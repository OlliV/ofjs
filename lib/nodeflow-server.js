...
export default class NodeFlowServer extends EventEmitter {
  ...
  start (address, port) {
    ...
    this.on('SENDPACKET', (obj) => {
      const sock = self.sessions.get(obj.packet.sessionID).sessionSocket;
      self.sendPacket(sock, obj.type, obj.packet.outmessage);
    });
  }
      ...
      switch (type) {
        ...
        case 'OFPT_PACKET_IN':
          ...
          this.emit('OFPT_PACKET_IN', {
            'message': obj.message,
            'sessionID': sessionID,
            'dpid': self.sessions.get(sessionID).dpid
          });
