...
module.exports.dump_bytes = function (rawPacket, offset) {
  for (var i = offset; i < rawPacket.length; i += 1) {
...
var unpack = {
  ethernet_addr: function (rawPacket, offset) {
    return [
      lpad(rawPacket[offset].toString(16), 2),
...
decode.packet = function (rawPacket) {
  let packet = {};
  // packet = decode.ethernet(rawPacket, 18);
  packet = decode.ethernet(rawPacket, 0);
  return packet;
};
...
decode.ethernet = function (rawPacket, offset) {
  let ret = {};
  ret.dhost = unpack.ethernet_addr(rawPacket, 0);
  ret.shost = unpack.ethernet_addr(rawPacket, 6);
  ret.ethertype = unpack.uint16(rawPacket, 12);
  offset = 14;
...
decode.vlan = function (rawPacket, offset) {
  ...
  ret.priority = (rawPacket[offset] & 0xE0) >> 5;
  ret.canonical_format = (rawPacket[offset] & 0x10) >> 4;
  ret.id = ((rawPacket[offset] & 0x0F) << 8) | rawPacket[offset + 1];
...
decode.arp = function (rawPacket, offset) {
  ...
  ret.htype = unpack.uint16(rawPacket, offset);
  // 0, 1
  ret.ptype = unpack.uint16(rawPacket, offset + 2);
  // 2, 3
  ret.hlen = rawPacket[offset + 4];
  ret.plen = rawPacket[offset + 5];
  ret.operation = unpack.uint16(rawPacket, offset + 6);
  // 6, 7
  if (ret.operation === 1) {
...
decode.ip = function (rawPacket, offset) {
  ...
  // first 4 bits
  ret.header_length = rawPacket[offset] & 15;
  // second 4 bits
  ret.header_bytes = ret.header_length * 4;
  ret.diffserv = rawPacket[offset + 1];
  ret.total_length = unpack.uint16(rawPacket, offset + 2);
  // 2, 3
  ret.identification = unpack.uint16(rawPacket, offset + 4);
  // 4, 5
  ret.flags = {};
  ret.flags.reserved = (rawPacket[offset + 6] & 128) >> 7;
  ret.flags.df = (rawPacket[offset + 6] & 64) >> 6;
  ret.flags.mf = (rawPacket[offset + 6] & 32) >> 5;
  ret.fragment_offset = ((rawPacket[offset + 6] & 31) * 256) + rawPacket[offset + 7];
  // 13-bits from 6, 7
  ret.ttl = rawPacket[offset + 8];
  ret.protocol = rawPacket[offset + 9];
  ret.header_checksum = unpack.uint16(rawPacket, offset + 10);
  // 10, 11
  ret.saddr = unpack.ipv4_addr(rawPacket, offset + 12);
  // 12, 13, 14, 15
  ret.daddr = unpack.ipv4_addr(rawPacket, offset + 16);
  // 16, 17, 18, 19
...
decode.udp = function (rawPacket, offset) {
  ...
  ret.sport = unpack.uint16(rawPacket, offset);
  // 0, 1
  ret.dport = unpack.uint16(rawPacket, offset + 2);
  // 2, 3
  ret.length = unpack.uint16(rawPacket, offset + 4);
  // 4, 5
  ret.checksum = unpack.uint16(rawPacket, offset + 6);
  // 6, 7
...
decode.tcp = function (rawPacket, offset, ip) {
  ...
  ret.dport = unpack.uint16(rawPacket, offset + 2);
  // 2, 3
  ret.seqno = unpack.uint32(rawPacket, offset + 4);
  // 4, 5, 6, 7
  ret.ackno = unpack.uint32(rawPacket, offset + 8);
  // 8, 9, 10, 11
  ret.data_offset = (rawPacket[offset + 12] & 0xf0) >> 4;
  // first 4 bits of 12
  ret.header_bytes = ret.data_offset * 4;
  // convenience for using data_offset
  ret.reserved = rawPacket[offset + 12] & 15;
  // second 4 bits of 12
  ret.flags = {};
  ret.flags.cwr = (rawPacket[offset + 13] & 128) >> 7;
  // all flags packed into 13
  ret.flags.ece = (rawPacket[offset + 13] & 64) >> 6;
  ret.flags.urg = (rawPacket[offset + 13] & 32) >> 5;
  ret.flags.ack = (rawPacket[offset + 13] & 16) >> 4;
  ret.flags.psh = (rawPacket[offset + 13] & 8) >> 3;
  ret.flags.rst = (rawPacket[offset + 13] & 4) >> 2;
  ret.flags.syn = (rawPacket[offset + 13] & 2) >> 1;
  ret.flags.fin = rawPacket[offset + 13] & 1;
  ret.window_size = unpack.uint16(rawPacket, offset + 14);
  // 14, 15
  ret.checksum = unpack.uint16(rawPacket, offset + 16);
  // 16, 17
  ret.urgent_pointer = unpack.uint16(rawPacket, offset + 18);
  // 18, 19
  ret.options = {};
