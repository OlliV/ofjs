...
module.exports = {
  unpack: function (buffer, offset) {
    ...
    const code = buffer.readUInt16BE(offset + offsets.code, true);

    if (ofputil.setEnum(message.body, 'type', type, ofp.ofp_error_type_rev)) {
      // NOTE: hackish
      const errorCodeMapName = message.body.type.toLowerCase().replace('ofpet_', 'ofp_') + '_code_rev';
    ...
    const dataLen = len - ofp.sizes.ofp_error_msg;
    if (dataLen > 0) {
