...
const offsets = ofp.offsets.ofp_match;
...
module.exports = {
  ...
  unpack: function (buffer, offset) {
    ...
    if (buffer.length < offset + ofp.sizes.ofp_match) {
      throw new Error(util.format('match at offset %d is too short (%d).',
    ...
    if ((wildcards & ofp.ofp_flow_wildcards.OFPFW_IN_PORT) === 0) {
      ...
      if (in_port > ofp.ofp_port.OFPP_MAX) {
        if (in_port === ofp.ofp_port.OFPP_LOCAL) {
          ...
          throw new Error(util.format('%s match at offset %d has invalid in_port (%d).', match.header.type, offset, in_port));
        }
      } else {
        match.in_port = in_port;
      }
  ...
  pack: function(match, buffer, offset) {
    if (buffer.length < offset + ofp.sizes.ofp_match) {
      throw new Error(util.format('match at offset %d does not fit the buffer.', offset));
    }
    ...
    if ('dl_vlan_pcp' in match.body) {
      dl_vlan_pcp = match.body.dl_vlan_pcp;
    } else {
      dl_vlan_pcp = 0;
