 ...
 * {"message" : <JSON>}
 */
Stream.prototype.processData = function processData (data, offset) {
  if (!offset) { offset = 0; }
  ...
  if (this.size === 0) {
    ...
    const msgSize = data.readUInt16BE(offset + ofp.offsets.ofp_header.length, true);
    ...
    const result = [oflib.unpack(data, offset)];

    /* Note: recursive call, internal data will be changed */
    return result.concat(this.processData(data, offset + msgSize));
  } else {
    ...
    const msgSize = this.buffer.readUInt16BE(offset + ofp.offsets.ofp_header.length, true);
    ...
    /* data.length - offset > msgSize */
    const result = [oflib.unpack(this.buffer, 0)];

    this.size -= msgSize;
    this.buffer.copy(this.buffer, 0, msgSize, msgSize + this.size);

    /* Note: recursive call, internal data will be changed */
    return result.concat(this.processData(this.EMPTY_BUFFER, 0));
  }
};
