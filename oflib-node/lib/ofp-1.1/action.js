  ...
  module.exports = {
    ...
    "pack" : function(action, buffer, offset) {
      ...
      switch (action.header.type) {
        ...
        case 'OFPAT_COPY_TTL_OUT': { return copyTtlOut.pack(action, buffer, offset); }
        case 'OFPAT_COPY_TTL_IN': { return copyTtlIn.pack(action, buffer, offset); }
        case 'OFPAT_SET_MPLS_LABEL': { return setMplsLabel.pack(action, buffer, offset); }
        case 'OFPAT_SET_MPLS_TC': { return setMplsTc.pack(action, buffer, offset); }
        case 'OFPAT_SET_MPLS_TTL': { return setMplsTtl.pack(action, buffer, offset); }
        case 'OFPAT_DEC_MPLS_TTL': { return decMplsTtl.pack(action, buffer, offset); }
        case 'OFPAT_PUSH_VLAN': { return pushVlan.pack(action, buffer, offset); }
        case 'OFPAT_PUSH_MPLS': { return pushMpls.pack(action, buffer, offset); }
        case 'OFPAT_POP_VLAN': { return popVlan.pack(action, buffer, offset); }
        case 'OFPAT_POP_MPLS': { return popMpls.pack(action, buffer, offset); }
        case 'OFPAT_SET_QUEUE': { return setQueue.pack(action, buffer, offset); }
        case 'OFPAT_GROUP': { return group.pack(action, buffer, offset); }
        case 'OFPAT_SET_NW_TTL': { return setNwTtl.pack(action, buffer, offset); }
        case 'OFPAT_DEC_NW_TTL': { return decNwTtl.pack(action, buffer, offset); }
        case 'OFPAT_EXPERIMENTER': { return experimenter.pack(action, buffer, offset); }
        default: {
          return {
            "error" : {
